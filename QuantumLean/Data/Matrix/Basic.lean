...
--
--
-- variable (o : Type*) (m n : F) [DecidableEq o]
--
--
-- def of : (∀ a b, m a → n b → α) ≃ Matrix' m n α :=
--   Equiv.refl _
--
--
--
-- def diagonal [Zero α] (d : o → α) : Matrix' m m α :=
...
def Matrix' (m n : F) (α : Type v) :=
  ∀ a b, m a → n b → α

variable {m n : F} {l o : Type*} {m' : o → Type*} {n' : o → Type*}
...
theorem ext_iff : (∀ x y i j, M x y i j = N x y i j) ↔ M = N :=
  ⟨fun h => funext fun x => funext fun y => funext fun i => funext <| h x y i, fun h => by simp [h]⟩

@[ext]
theorem ext : (∀ x y i j, M x y i j = N x y i j) → M = N :=
  ext_iff.mp
...
@[simp]
theorem of_apply (f : ∀ a b, m a → n b → α) (i j) : of f i j = f i j :=
  rfl
...
@[simp]
theorem map_apply {M : Matrix' m n α} {f : α → β} {x y} {i j} : M.map f x y i j = f (M x y i j) :=
  rfl
...
theorem map_injective {f : α → β} (hf : Function.Injective f) :
    Function.Injective fun M : Matrix' m n α => map M f := fun _ _ h =>
  ext fun x y i j => hf <| ext_iff.mpr h x y i j

/-- The transpose of a matrix. -/
def transpose (M : Matrix' m n α) : Matrix' n m α :=
  of fun x y i j => M y x j i

-- TODO: set as an equation lemma for `transpose`, see mathlib4#3024
@[simp]
theorem transpose_apply (M : Matrix' m n α) (x y i j) : M.transpose x y i j = M y x j i :=
  rfl
...
instance inhabited [Inhabited α] : Inhabited (Matrix' m n α) :=
  inferInstanceAs <| Inhabited <| ∀ i j, m i → n j → α

-- Porting note: new, Lean3 found this automatically
-- instance decidableEq [DecidableEq α] [Fintype m.toType] [Fintype n.toType] : DecidableEq (Matrix' m n α) :=
...
instance subsingleton [Subsingleton α] : Subsingleton (Matrix' m n α) :=
  inferInstanceAs <| Subsingleton <| ∀ i j, m i → n j → α

-- instance nonempty [Nonempty m.toType] [Nonempty n.toType] [Nontrivial α] : Nontrivial (Matrix' m n α) :=
--   Function.nontrivial
...
@[simp]
theorem zero_apply [Zero α] (i j) : (0 : Matrix' m n α) x y i j = 0 := rfl

@[simp]
theorem add_apply [Add α] (A B : Matrix' m n α) (i j) :
    (A + B) x y i j = (A x y i j) + (B x y i j) := rfl

@[simp]
theorem smul_apply [SMul β α] (r : β) (A : Matrix' m n α) (i j) :
    (r • A) i j = r • (A i j) := rfl

@[simp]
theorem sub_apply [Sub α] (A B : Matrix' m n α) (i j) :
    (A - B) x y i j = (A x y i j) - (B x y i j) := rfl

@[simp]
theorem neg_apply [Neg α] (A : Matrix' m n α) (i j) :
    (-A) x y i j = -(A x y i j) := rfl
...
@[simp]
theorem of_zero [Zero α] : of (0 : ∀ a b, m a → n b → α) = 0 :=
  rfl

@[simp]
theorem of_add_of [Add α] (f g : ∀ a b, m a → n b → α) : of f + of g = of (f + g) :=
  rfl

@[simp]
theorem of_sub_of [Sub α] (f g : ∀ a b, m a → n b → α) : of f - of g = of (f - g) :=
  rfl

@[simp]
theorem neg_of [Neg α] (f : ∀ a b, m a → n b → α) : -of f = of (-f) :=
  rfl

@[simp]
theorem smul_of [SMul R α] (r : R) (f : ∀ a b, m a → n b → α) : r • of f = of (r • f) :=
  rfl
...
theorem _root_.IsSMulRegular.Matrix' [SMul R S] {k : R} (hk : IsSMulRegular S k) :
  ...
  IsSMulRegular.pi fun _ => IsSMulRegular.pi fun _ => IsSMulRegular.pi fun _ => IsSMulRegular.pi fun _ => hk
...
-/
def diagonal [Zero α] (d : ∀ a, n a → α) : Matrix' n n α :=
  of fun x y i j => if i = j then d x i else 0

-- TODO: set as an equation lemma for `diagonal`, see mathlib4#3024
theorem diagonal_apply [Zero α] (d : n → α) (i j) : diagonal d i j = if i = j then d x i else 0 :=
  rfl
