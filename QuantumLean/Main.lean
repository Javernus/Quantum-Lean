...
theorem HZHeqX : Hadamard n * ZGate n * Hadamard n = (2 ^ n : ℕ) • XGate n := by
  induction n with
    ...
    | succ n ih =>
      ...
      rw [← mul_kronecker_mul, ← mul_kronecker_mul, ih, smul_kronecker]
      ...
      rw [show (!![0, 2; 2, 0] : Matrix (Fin 2) (Fin 2) ℂ) = 2 • !![0, 1; 1, 0] by norm_num]
      rw [kronecker_smul]
...
theorem HXHeqZ : Hadamard n * XGate n * Hadamard n = (2 ^ n : ℕ) • ZGate n := by
  induction n with
    ...
    | succ n ih =>
      ...
      rw [show (!![1 + 1, 0; 0, -1 + -1] : Matrix (Fin 2) (Fin 2) ℂ) = (2 : ℕ) • !![1, 0; 0, -1] by norm_num]
      rw [kronecker_smul]
