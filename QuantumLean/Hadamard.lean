...
theorem reindexHadamard_mul (A B : Matrix (Fin (2 ^ n) × Fin 2) (Fin (2 ^ n) × Fin 2) ℤ) : reindexHadamard (A * B) = reindexHadamard A * reindexHadamard B :=
  Matrix.submatrix_mul _ _ _ _ _ (finProdFinEquiv.symm.bijective)


theorem reindexHadamard_natCast (m : ℕ) :
    reindexHadamard (m : Matrix (Fin (2 ^ n) × Fin 2) (Fin (2 ^ n) × Fin 2) ℤ) = m := by
  ext i j
...
theorem kronecker_natCast_natCast {A B : Type} [DecidableEq A] [DecidableEq B] (m₁ m₂ : ℕ) :
    (m₁ : Matrix A A ℤ) ⊗ₖ (m₂ : Matrix B B ℤ) = ((m₁ * m₂ : ℕ) : Matrix (A × B) (A × B) ℤ) := by
  simp_rw [← nsmul_one]
...
theorem HnxHn : Hadamard n * Hadamard n = (2 ^ n : ℕ) := by
  induction n with
    ...
    | succ n ih =>
      ...
      rw [show (!![1, 1; 1, -1]) * (!![1, 1; 1, -1]) = ((2 : ℕ) : Matrix (Fin 2) (Fin 2) ℤ) by decide]
